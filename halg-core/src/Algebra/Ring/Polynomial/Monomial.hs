...
instance M.MVector UV.MVector WrapOrdering where
  ...
  basicUnsafeNew n = MV_WrapOrdering <$> M.basicUnsafeNew n
  basicInitialize (MV_WrapOrdering v) = M.basicInitialize v
  basicUnsafeReplicate n x = MV_WrapOrdering <$> M.basicUnsafeReplicate n (fromWrapOrdering x)
  basicUnsafeRead (MV_WrapOrdering v) i = toWrapOrdering <$> M.basicUnsafeRead v i
  ...
  basicUnsafeGrow (MV_WrapOrdering v) n = MV_WrapOrdering <$> M.basicUnsafeGrow v n
...
instance G.Vector UV.Vector WrapOrdering where
  ...
  {-# INLINE elemseq #-}
  basicUnsafeFreeze (MV_WrapOrdering v) = V_WrapOrdering <$> G.basicUnsafeFreeze v
  basicUnsafeThaw (V_WrapOrdering v) = MV_WrapOrdering <$> G.basicUnsafeThaw v
  ...
  basicUnsafeIndexM (V_WrapOrdering v) i = toWrapOrdering <$> G.basicUnsafeIndexM v i
...
grevlexF = (Fl.purely ofoldlUnwrap body .) . (UV.zip `on` V.unsized)
  where
    ...
    body =
      (<>) <$> Fl.foldMap (DC.coerce @_ @(Sum Int, Sum Int)) (uncurry compare)
