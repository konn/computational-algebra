  ...
  defaultMain
    ...
    , bgroup
        ...
        [ bgroup
            ...
            , env
                ( (,) <$> generateFrom rat_100_complex_a
                    <*> return (const $ Scalar (42424242 :: Rational))
                )
                $ \ ~((a, b, c), inp) ->
                  bgroup
            ...
            , env
                ( (,) <$> generateFrom rat_1000_simple_a
                    <*> return (const $ Scalar (42424242 :: Rational))
                )
                $ \ ~((a, b, c), inp) ->
                  bgroup
...
generateFrom ::
  (Eq k, Field k) =>
  [k] ->
  IO (Unipol k, OGUnipol k, OLUnipol k)
generateFrom cfs = do
