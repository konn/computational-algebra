...
instance (NA.Ring r, Eq r) => NA.Commutative (Polynomial r)
instance (NA.Ring r, Eq r) => NA.Multiplicative (Polynomial r) where
...
instance (Eq r, NA.Ring r) => NA.Ring (Polynomial r)
instance (Eq r, NA.Ring r) => NA.Group (Polynomial r) where
  negate (Polynomial dic) = Polynomial $ fmap NA.negate dic
instance (Eq r, NA.Ring r) => NA.Rig (Polynomial r)
instance (Eq r, NA.Ring r) => NA.Unital (Polynomial r) where
...
instance (Eq r, NA.Ring r) => NA.Abelian (Polynomial r)
instance (Eq r, NA.Ring r) => NA.Additive (Polynomial r) where
  (Polynomial f) + (Polynomial g) = normalize $ Polynomial $ M.unionWith (NA.+) f g

instance (Eq r, NA.Ring r) => NA.LeftModule (Scalar r) (Polynomial r) where
...
instance (Euclidean a, Integral a, Show a) => Show (Polynomial (Fraction a)) where
  show = showRatPolynomial

instance (NA.DecidableZero r, Eq r, NA.Ring r, Show r) => Show (Polynomial r) where
  show = showPolynomial

instance (NA.DecidableZero r, Eq r, NA.Ring r, Poly.IsMonomialOrder ord)
...
uniformlyPromoteWithDim :: (NA.DecidableZero r, Eq r, NA.Ring r)
...
uniformlyPromote :: (NA.DecidableZero r, Eq r, NA.Ring r, Poly.IsMonomialOrder ord)
...
promoteList :: (Eq r, NA.Ring r, NA.DecidableZero r, Poly.IsMonomialOrder ord)
            => [Polynomial r] -> Monomorphic ([] :.: Poly.OrderedPolynomial r ord)
promoteList ps = promoteListWithDim (length vars) ps
  where
    vars = nub $ sort $ concatMap buildVarsList ps

promoteListWithVarOrder :: (NA.DecidableZero r, Eq r, NA.Ring r, Poly.IsMonomialOrder ord)
...
promoteListWithDim :: (NA.DecidableZero r, Eq r, NA.Ring r, Poly.IsMonomialOrder ord)
...
showPolynomial :: (NA.DecidableZero r, Show r, Eq r, NA.Ring r) => Polynomial r -> String
showPolynomial f =
...
showRatPolynomial :: (Eq a, Ord a, Euclidean a, Show a) => Polynomial (Fraction a) -> String
showRatPolynomial f =
  case encodePolynomial f of
    Monomorphic f' ->
        case singInstance (Poly.sArity f') of
