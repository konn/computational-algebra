...
instance (SingI n, ToWeightVector ns) => ToWeightVector (n ': ns) where
...
diff :: forall n m ord r. (Ring r, SingI n, IsMonomialOrder ord, SingI m, (m :<<= n) ~ True)
...
changeOrder :: (Eq (Monomial n), IsOrder o, IsOrder o',  SingI n)
            => o' -> OrderedPolynomial k o n -> OrderedPolynomial k o' n
changeOrder _ = unwrapped %~ M.mapKeys (OrderedMonomial . getMonomial)

changeOrderProxy :: (Eq (Monomial n), IsOrder o, IsOrder o',  SingI n)
