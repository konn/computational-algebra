...
intersect i j
  | disjoint i j = Interval zero zero
    ...
    let i' = until (not . (q `isIn`)) (improveWith sf) i
     in if q <= lower i'
          then LT
            ...
            if isZero (f `modPolynomial` [var 0 - injectCoeff q])
              then EQ
              else GT
  ...
  let (p, q) = (F.numerator r, F.denominator r)
   ...
   in case (exactSquareRoot p, exactSquareRoot q) of
        ...
        (mp', mq') ->
          Right $
            Interval
              (fromMaybe isp mp' F.% fromMaybe (isq + 1) mq')
...
nthRootRat 4 r =
  let (p, q) = (F.numerator r, F.denominator r)
      (isp, isq) = (integerRoot 4 p, integerRoot 4 q)
   in case (exactRoot 4 p, exactRoot 4 q) of
        (Just p', Just q') -> Left (p' F.% q')
        (mp', mq') ->
          Right $
            Interval
...
nthRootRatCeil 3 r =
  let p = integerCubeRoot (F.numerator r) + 1
   ...
   in p F.% q
nthRootRatCeil 4 r =
  let p = integerRoot 4 (F.numerator r) + 1
      q = integerRoot 4 (F.denominator r)
   in p F.% q
nthRootRatCeil n r =
...
nthRootRatFloor 3 r =
  let p = integerCubeRoot (F.numerator r)
   ...
   in p F.% q
nthRootRatFloor 4 r =
  let p = integerRoot (4 :: Int) (F.numerator r)
      q = integerRoot (4 :: Int) (F.denominator r) + 1
   in p F.% q
nthRootRatFloor n r =
  ...
  let ts = terms g
      ...
      loop m k =
        let deg = totalDegree m
         in Add $
              toPolynomial
                ( injectCoeff k * #x ^ P.fromIntegral deg
...
recipInt i
  | lower i < 0 = Interval (recip $ upper i) (recip $ lower i)
   ...
   in runAdd $
        ifoldMap
          ( \m k ->
              Add $
                toPolynomial
                  (k, OrderedMonomial $ singleton $ d - totalDegree m)
          )
          ts
...
powA r n
  | n < 0 = nthRoot' (abs $ fromIntegral n) r
  | n == 0 = Rational 1
