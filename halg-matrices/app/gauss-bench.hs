...
instance
  (KnownNat r, KnownNat n, KnownNat m, DecidableZero a, Arbitrary a) =>
  Arbitrary (Sparse r n m a)
  where
  arbitrary =
...
getMatrix ::
  (Show a, Read a, Field a, Arbitrary a) =>
  String ->
  proxy a ->
  Double ->
  Int ->
...
mkCases ::
  ...
  , Eq a
  , Matrix DM.DMatrix a
  , Matrix IM.IMMatrix a
  , Matrix RIM.DRIMMatrix a
  ) =>
  String ->
  IO [Vector a] ->
  Benchmark
mkCases lab mrows =
  env
    ( do
        rows <- mrows
        return
          ( fromRows @DM.DMatrix rows
          , fromRows @IM.IMMatrix rows
          , fromRows @RIM.DRIMMatrix rows
          )
    )
    $ \ ~(dm, im, drim) ->
      bgroup
