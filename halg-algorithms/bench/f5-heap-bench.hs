...
import qualified Data.Heap.Pairing                       as HP
import qualified Data.Heap.Spray                         as HS
import           Gauge.Main
...
mkTC name jdeal =
  ...
  bgroup name [ bench "F5-orig"    $ nf (f5With (Proxy :: Proxy HE.Heap))    ideal
              , bench "F5-spray"   $ nf (f5With (Proxy :: Proxy HS.Spray))   ideal
              , bench "F5-pairing" $ nf (f5With (Proxy :: Proxy HP.Pairing)) ideal
              ]
