...
idealOfArity ::
  (KnownNat n, Eq k, NA.Field k, Arbitrary k) =>
  SNat n ->
  Gen (Ideal (Polynomial k n))
...
instance
  (Monad m, Serial m k, CoeffRing k, KnownNat n, IsMonomialOrder n ord) =>
  Serial m (OrderedPolynomial k ord n)
  where
